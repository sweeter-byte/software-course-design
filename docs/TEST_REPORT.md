...
- 10 个测试文件通过
- 44 个测试用例通过
- 覆盖链路：健康检查、认证、资料维护、双验证码改手机号、改密、找回密码、注销、课程 CRUD、多条件查询、选课、课程反馈新增/修改/查看/删除与仪表盘统计、作业发布/修改/取消、学生提交状态和个人提交摘要、截止/已提交后的作业修改保护、提交/修改/查询/批改、问题反馈发布/总览（默认上限 100/最大 200/offset 分页/非法 limit 回退/状态大小写归一化/角色权限边界）/修改/删除、回复新增/修改/删除、摘要
...
- 13 个测试文件通过
- 49 个测试用例通过
- 覆盖链路：运行态会话恢复、展示文案、依赖解析、无请求体 API 写操作请求头、反馈线程总览 API 查询参数、日期转换、友好错误文案、演示默认作业日期保持在未来、学生已批改提交只读与按钮文案、未提交/已提交三态按钮切换、批改前后反馈表单可用性、上下文栏学生提交注入与无提交时禁用、教师待批改卡片点击与回调、查看对应提交跳转、教师任务详情面板加载/缺失态、课程反馈展示带姓名/学号、登录/注册/找回密码 authMode 切换、账号资料/密码/手机号子表单回调、`resolveWorkspaceContext` 学生与教师两路解析
...
- Web 已补统一上下文、学生“我的作业”和教师任务工作台；登录页和账号维护已剥离到独立组件目录（`features/auth/*`、`features/account/*`），教师任务台再拆为 `PendingSubmissionList / SubmissionDetailCard / FeedbackThreadList / CourseFeedbackOverview` 四个子组件。
- `/feedbacks/threads` 已加默认 `limit=100`、最大 200 上限和 `offset` 分页，超出和非法值会回退到默认值。
