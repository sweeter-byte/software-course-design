...
- 10 个测试文件通过
- 40 个测试用例通过
- 覆盖链路：健康检查、认证、资料维护、双验证码改手机号、改密、找回密码、注销、课程 CRUD、多条件查询、选课、课程反馈新增/修改/查看/删除与仪表盘统计、作业发布/修改/取消、学生提交状态、截止/已提交后的作业修改保护、提交/修改/查询/批改、问题反馈发布/修改/删除、回复新增/修改/删除、摘要
...
- 5 个测试文件通过
- 8 个测试用例通过
- 覆盖链路：运行态会话恢复、展示文案、依赖解析、无请求体 API 写操作请求头、演示默认作业日期保持在未来
...
- Web 和 Mobile 已补齐验收所需入口，但仍是工作台式集中页面，后续可以拆成更细的业务页面。
